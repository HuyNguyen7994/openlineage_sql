...
use sqlparser::ast::{
    ...
    TableFactor, TableFunctionArgs, TableObject, UpdateTableFromKind, Use, Value, ValueWithSpan,
    ...
    fn visit(&self, context: &mut Context) -> Result<()> {
        match self {
            TableFactor::Table {
                ...
                if !context.is_main() {
                    context.add_table_dependency(effective_name.to_idents());
                }

                if context.is_main() && !context.has_alias(effective_name.to_string()) {
                    context.add_input(effective_name.to_idents());
                }

                if context.is_main() && context.has_alias(effective_name.to_string()) {
                    context.mark_table_as_used(effective_name.to_idents());
                }
        ...
        match self {
            ...
            Expr::Case {
                ...
                for condition in conditions {
                    condition.condition.visit(context)?;
                    condition.result.visit(context)?;
                }
...
impl Visit for Select {
    fn visit(&self, context: &mut Context) -> Result<()> {
        ...
        if let Some(into) = &self.into {
            context.add_output(into.name.to_idents())
        }
    ...
    fn visit(&self, context: &mut Context) -> Result<()> {
        ...
        match self {
            ...
            Statement::Insert(insert) => {
                ...
                match &insert.table {
                    TableObject::TableName(name) => context.add_output(name.to_idents()),
                    _ => panic!("Table function not implemented yet"),
                };
            }
            ...
            Statement::CreateTable(ct) => {
                ...
                if let Some(like_table) = &ct.like {
                    context.add_input(like_table.to_idents());
                }
                if let Some(clone) = &ct.clone {
                    context.add_input(clone.to_idents());
                }

                context.add_output(ct.name.to_idents());
            }
            Statement::CreateView { name, query, .. } => {
                query.visit(context)?;
                context.add_output(name.to_idents());
            }
            Statement::CreateStage {
                ...
                context.add_non_table_output(name.to_idents(), false, true);
            }
            Statement::Update {
                table,
                assignments: _,
                ...
                for operation in operations {
                    match operation {
                        AlterTableOperation::SwapWith { table_name } => {
                            ...
                            context.add_input(table_name.to_idents());
                            context.add_input(name.to_idents());

                            context.add_output(table_name.to_idents());
                            context.add_output(name.to_idents());
                        }
                        ...
                        _ => context.add_output(name.to_idents()),
                    }
                }
            }
            ...
            Statement::Truncate { table_names, .. } => {
                for table_name in table_names {
                    context.add_output(table_name.name.to_idents())
                }
            }
            ...
            Statement::CopyIntoSnowflake { into, from_obj, .. } => {
                context.add_output(into.to_idents());
                if from_obj.clone().unwrap().to_string().contains("gcs://")
                    || from_obj.clone().unwrap().to_string().contains("s3://")
                    || from_obj.clone().unwrap().to_string().contains("azure://")
                {
                    context.add_non_table_input(
                        vec![Ident::new(
                            from_obj
                    ...
                    // Stage
                    context.add_non_table_input(from_obj.clone().unwrap().to_idents(), true, true);
                };
            }
...
fn get_table_name_from_table_factor(table: &TableFactor, context: &Context) -> Option<Vec<Ident>> {
    if let TableFactor::Table { name, args, .. } = table {
        if is_identifier_function_used_to_provide_table_name(name, args, context) {
            get_table_name_from_identifier_function(args).map(|table_name| table_name.to_idents())
        } else {
            Some(name.to_idents())
        }
    } else {
...
fn is_identifier_function_used_to_provide_table_name(
    ...
    (context.dialect().as_base().is::<DatabricksDialect>()
        ...
        && name.0.first().unwrap().to_string().to_lowercase() == "identifier"
        && args.is_some()
}
...
pub fn extract_up_to_two_ident_values(
    ...
    let first_value = idents_iter
        .next()
        .map(|ident| ident.as_ident().unwrap().value.clone());
    let second_value = idents_iter
        .next()
        .map(|ident| ident.as_ident().unwrap().value.clone());
